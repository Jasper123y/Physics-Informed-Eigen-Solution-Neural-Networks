...
def load_geometry_images(
    ...
    max_samples : int, optional
        Maximum number of samples to load. If None, load all (default: None).
    ...
    # Sort numerically - handles both "0001.txt" and "name_0001.txt" formats
    def extract_number(filename):
        base = filename.replace('.txt', '')
        parts = base.split('_')
        # Check last part first (for "prefix_0001.txt" format)
        if parts[-1].isdigit():
            return int(parts[-1])
        # Otherwise try the whole base (for "0001.txt" format)
        if base.isdigit():
            return int(base)
        return 0
...
def load_mode_shapes(
    ...
    """
    data_files = [f for f in os.listdir(folder_path) if f.endswith('.txt')]
    # Sort numerically - handles both "0001.txt" and "name_0001_mode.txt" formats
    def extract_number(filename):
        # Remove extension and try to extract number
        base = filename.replace('.txt', '')
        # Try to get number from filename (handles "0001" or "prefix_0001_suffix")
        parts = base.split('_')
        for part in parts:
            if part.isdigit():
                return int(part)
        # If no underscore-separated number, try the whole base
        if base.isdigit():
            return int(base)
        return 0
...
def load_all_mode_shapes(
    ...
    for folder_path in folder_paths:
        modes = load_mode_shapes(
            folder_path, image_shape, transpose, add_channel_dim, max_samples
        )
